...
function eliminarData(id) {
	...
	if (obj.ieps != "") {
		totalIEPS -= redondearValor((obj.ieps / 100) *  (obj.valorUnitario * obj.cantidad));
	}
...
function agregarConcepto() {
	...
	if ($('#ivaIncluido').is(':checked')) {
		// var ivaIncluido = $('#valorUnitario').val() / (1 + ($('#iva').val())
		// 100);
		var porIva = (1 + ($('#iva').val()) / 100);
		
		if ($('#ish').is(':checked')) {
		//	obj.valorUnitario -= (obj.valorUnitario * 0.02);
			var tasaISH = parseFloat($('#tasaISH').val());
			porIva += tasaISH; // 0.02;
		}
		obj.valorUnitario = redondearValor($('#valorUnitario').val() / porIva);
		// obj.valorUnitario = precise_round(($('#valorUnitario').val() / porIva), 2);
	}
	...
	if (obj.ivaExento == false || obj.ivaExento == undefined) {
		...
		iva = redondearValor((obj.cantidad * (obj.valorUnitario - descuento)) * (obj.iva / 100));
		totalIVA += iva;
		obj.iva = iva;
...
function comprobanteToJSON() {
	...
	if (mapImpuestosTraslados.length > 0) {
		...
		traslado.tasa = parseFloat($('#ieps').val());
		...
		traslado.importe = redondearValor(ivaTras + ivaIEPS);
...
function graficarConceptos() {
	...
	if (totalIEPS != 0 && $('#ivaIeps').val() != "false") {
		ivaIEPS = (totalIEPS * (parseFloat($('#iva').val()) / 100));
		totalIVA += ivaIEPS;
		...
		.ready(
				function() {
					...
					$('input[name=tasade]').change(function() {  
						 ...
						 var result = (valor * $('input[name=subTotal]').val()) / 100 ;
