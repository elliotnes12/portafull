	...
	function cancelarCfd(id, fecha, rfcReceptor, total, uuid) {
		if (confirm("¿Desea cancelar el folio? ")) {
			...
			$.get(context + "/rest/comprobante/cancelarNuevoEsquema/" + id + "/" + fecha + "/" + rfcReceptor + "/" + redondearValor(total).toFixed(2) + "/" + uuid, function(cancelado) {
				if (cancelado == "Cancelado sin aceptación" || cancelado == "En proceso Cancelable sin aceptación" || cancelado == "En proceso Cancelable con aceptación" || cancelado == "Cancelado Plazo vencido") {
					alert(cancelado);
					...
					if(cancelado.indexOf("No Cancelable") != -1){
						...
						if(cancelado.indexOf("rel") != -1)
						{
                           ...
                           if(angular.isNumber(i)){
		                      ...
		                      /*
		                       Eliminando caracteres y elementos de más 
		                      */
		                      strc = strc.replace("\\","");
		                      strc = strc.replace("[","");
		                      strc = strc.replace("]","");
			...
			<table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered table-hover" id="listaCfd" style="font-size: 12px">
				<thead>
					<tr class="filterHeader">
						...
						<th rowspan="1" colspan="1"><input class="search_init span1" type="text" name="search_version" placeholder="Creación" value="" /></th>
				...
				<tr id="row-${cfd.id}">
					...
					<td class="images">
						...
						<a title="Cancelación CFDI 2018" href="#" onclick="cancelarCfd(${cfd.id}, '${cfd.fechaRegistro.toString().substring(0,7)}', '${cfd.rfcReceptor}', '${cfd.total}', '${cfd.uuid}')">
